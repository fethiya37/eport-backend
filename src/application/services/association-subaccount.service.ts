...
import {type IAssociationSubaccountRepository, ASSOCIATION_SUBACCOUNT_REPOSITORY } from '../../domain/repositories/association-subaccount.repository';
...
export class AssociationSubaccountService {
  ...
  private resolveAssociationId(ctx: UserContext, association_id?: number) {
    if (isAdminLike(ctx.user_type)) {
      if (!association_id) throw new BadRequestException('association_id is required for admin/superadmin');
