...
export class PrismaDriverRepository implements IDriverRepository {
  ...
  async create(
    ...
    return tx.driver.create({
      data: {
        ...
        has_smartphone: data.has_smartphone ?? true,  // ✅ default true
      },
    });
  }
  ...
  async findAll(ctx: UserContext, filter?: DriverFilter): Promise<Driver[]> {
    ...
    const where: Prisma.DriverWhereInput = {
      ...baseScope,
      ...(isAdminLike(ctx.user_type) && filter?.association_id ? { association_id: filter.association_id } : {}),
      ...
      ...(filter?.has_smartphone !== undefined ? { has_smartphone: filter.has_smartphone } : {}), // ✅ filter
    };
  ...
  async findWithoutVehicle(ctx: UserContext): Promise<Driver[]> {
    ...
    return this.prisma.driver.findMany({
      where: {
        ...baseScope,
        vehicle: null,   // ✅ no linked vehicle
      },
      orderBy: { id: 'asc' },
    });
  }
