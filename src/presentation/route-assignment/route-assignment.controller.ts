...
export class RouteAssignmentController {
  ...
  updateOne(@AuthUser() user: UserContext, @Param('id', ParseIntPipe) id: number, @Body() dto: UpdateAssignmentDto) {
    return this.service.updateOne(user, id, dto);
  }

  /** Visible current→future window for a driver by plate_number, only if coverage active. */
  @Get('visible-coverage')
  @Roles('Driver','Owner','Controller')
  @ApiOperation({
    summary: 'View route assignments from current period start → active_until_date (Approved only), GC in/out, by plate_number',
  })
  visibleCoverage(@AuthUser() user: UserContext, @Query() q: VisibleCoverageQueryDto) {
    return this.service.visibleCoverage(user, q.plate_number);
  }
}
