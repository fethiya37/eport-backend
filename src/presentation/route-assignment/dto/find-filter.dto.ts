...
export class RouteAssignmentFilterDto {
  ...
  @ApiPropertyOptional({ enum: RouteAssignmentStatus }) @IsOptional() @IsEnum(RouteAssignmentStatus) status?: RouteAssignmentStatus;
  @ApiPropertyOptional() @IsOptional() @IsBoolean() is_weekly?: boolean;

  @ApiPropertyOptional({ example: '2017-01-01' })
  @IsOptional() @IsString() @Matches(/^\d{4}-\d{2}-\d{2}$/) date_from?: string;

  @ApiPropertyOptional({ example: '2017-01-07' })
  @IsOptional() @IsString() @Matches(/^\d{4}-\d{2}-\d{2}$/) date_to?: string;
